...
run-local:
	docker compose -p log4fit-local -f docker-compose.local.yml up --build

run-prod:
	docker compose -p log4fit-prod --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml up --build

run-dev:
	docker compose -p log4fit-dev --env-file .env.dev -f docker-compose.dev-db.yml -f docker-compose.dev.yml up --build
